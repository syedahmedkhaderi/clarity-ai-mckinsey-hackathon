  ...
  return (
    <div
      className={clsx(
        ...
        highlighted ? "border-agent-line" : "border-line",
      )}
    >
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        <Tag tone="agent">{ACTION_LABELS[action.type] ?? action.type}</Tag>
        {changed === "added" && <Tag tone="agent">New</Tag>}
        {changed === "rescheduled" && <Tag tone="agent">Moved up</Tag>}
        <span className="ml-auto">
          <Priority severity={action.severity} />
        </span>
      </div>
...
export function FeedbackChecks({
  ...
  return (
    <div className="rounded-md border border-line bg-surface">
      ...
      >
        <Tag tone="agent">{ACTION_LABELS.feedback_review}</Tag>
        <span className="text-sm font-medium text-ink">{title}</span>
        <span className="ml-auto w-10 text-right text-xs text-ink-muted">{open ? "Hide" : "Show"}</span>
      </button>
      {open && (
      ...
      <ul className="divide-y divide-line">
        {shown.map((c, i) => (
          <li key={i} className="flex gap-3 px-4 py-2 text-sm">
            <Tag
              tone={c.kind === "added" || c.kind === "rescheduled" ? "agent" : "neutral"}
              className="mt-0.5 shrink-0 self-start"
            >
              {PLAN_CHANGE_PLAIN_LABELS[c.kind]}
            </Tag>
            <span className="text-ink-muted">{plain(planChangeTitle(c.detail))}</span>
          </li>
        ))}
      </ul>
    </div>
